...
final private[opencensus] case class OpenCensusSpan[F[_]: Sync](tracer: Tracer, span: io.opencensus.trace.Span)
    extends Span[F] {
  ...
  override def put(fields: (String, TraceValue)*): F[Unit] =
    fields.toList.traverse_ {
      case (k, StringValue(v)) =>
        Sync[F].delay(span.putAttribute(k, AttributeValue.stringAttributeValue(v)))
      case (k, NumberValue(v)) =>
        Sync[F].delay(span.putAttribute(k, AttributeValue.doubleAttributeValue(v.doubleValue())))
      case (k, BooleanValue(v)) =>
        Sync[F].delay(span.putAttribute(k, AttributeValue.booleanAttributeValue(v)))
    }
  ...
  override def span(name: String): Resource[F, Span[F]] =
    Resource
      .makeCase(OpenCensusSpan.child(this, name))(OpenCensusSpan.finish)
      .widen
}
