...
object Jaeger {
  ...
  def entryPoint[F[_]: Sync](system: String)(
    ...
    Resource.make(Sync[F].delay(new Configuration(system)).flatMap(configure))(c => Sync[F].delay(c.close)).map { t =>
      new EntryPoint[F] {
        ...
        def root(name: String): Resource[F, Span[F]] =
          Resource.make(Sync[F].delay(t.buildSpan(name).start()))(s => Sync[F].delay(s.finish)).map(JaegerSpan(t, _))
