...
lazy val opentelemetry = project
  ...
  .settings(
    ...
    tlVersionIntroduced := List("2.12", "2.13", "3").map(_ -> "0.1.7").toMap,
    libraryDependencies ++= Seq(
      "io.opentelemetry" % "opentelemetry-sdk" % "1.60.0"
    )
  )
...
lazy val datadog = project
  ...
  .settings(
    ...
    libraryDependencies ++= Seq(
      "com.datadoghq" % "dd-trace-ot" % "1.56.2",
      "com.datadoghq" % "dd-trace-api" % "1.56.2"
    )
  )
...
lazy val examples = project
  ...
  .settings(
    ...
    libraryDependencies ++= Seq(
      ...
      "is.cir" %% "ciris" % "3.12.0",
      "io.opentelemetry" % "opentelemetry-exporter-otlp" % "1.60.0",
      "io.opentelemetry.semconv" % "opentelemetry-semconv" % "1.40.0",
      "io.grpc" % "grpc-okhttp" % "1.79.0" // required for the OpenTelemetry exporter
    )
  )
...
lazy val docs = project
  ...
  .settings(
    ...
    libraryDependencies ++= Seq(
      ...
      "org.slf4j" % "slf4j-simple" % "2.0.17",
      "io.opentelemetry" % "opentelemetry-exporter-otlp" % "1.60.0", // for the opentelemetry example
      "io.opentelemetry.semconv" % "opentelemetry-semconv" % "1.40.0"
    ),
